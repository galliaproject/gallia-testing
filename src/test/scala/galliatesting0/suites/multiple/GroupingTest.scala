...
// ===========================================================================
object GroupingTest extends gallia.testing.Suite with gallia.testing.More {
  import TestDataO._
  import TestDataS._
  ...
  override def test(): Unit = {
    ...
    // ---------------------------------------------------------------------------
    private def _test(inMemory: Boolean) { implicit def idef220705141138(values: BObjs): AObjs = values.forceAObjs
      ...
      def alter(values: AObjs)(key: Key): AObjs = if (inMemory) values else values.sortBy(key)._forceResult
