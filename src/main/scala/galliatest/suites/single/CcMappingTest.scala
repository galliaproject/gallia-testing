...
object CcMappingTest extends gallia.testing.Suite {
  ...
  override def test() {
    ...
    Default01   .transform(_.string('f)).using { s => s.as.noneIf(_.startsWith("z")).map(s2 => Foo(s2, s2.toUpperCase)) }.check(
      aobj(
        cls('f  .cls_('a.string, 'A.string), 'g.int))(
        obj('f -> obj('a -> "foo", 'A -> "FOO"), 'g -> 1)))
  
    // will remove it
    Default01   .transform(_.string('f)).using { s => s.as.noneIf(_.startsWith("f")).map(s2 => Foo(s2, s2.toUpperCase)) }.check(
      aobj(
        cls('f  .cls_('a.string, 'A.string), 'g.int))(
        obj(                                 'g -> 1)) )
	}
