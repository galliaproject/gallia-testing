...
object SquashingTest extends gallia.testing.Suite {
  ...
  override def test() {
    ...
    Default03.transform(_.obj('p)).using(_.squash(_.string('f), _.int('g)).using((f, g) => f.size + g)            ).check(bobj('p -> 4, 'z -> true))
    Default03.transform(_.obj('p)).using(_.squash(_.string('f), _.int('g)).using((f, g) => f.size + g).mapV(_ + 1)).check(bobj('p -> 5, 'z -> true))
    Default03.transform(_.obj('p)).using(_.squashUnsafe(o => o.string('f).size + o.int('g)           )            ).check(bobj('p -> 4, 'z -> true))
    Default03.transform(_.obj('p)).using(_.squashUnsafe(o => o.string('f).size + o.int('g)           ).mapV(_ + 1)).check(bobj('p -> 5, 'z -> true))
...
//TODO: int vs ints inconsistency - also vs ooo
  ...
  GrabIn.transform(_.objz('a)).using(_.grab(_.int('h))).check(GrabOut)
  GrabIn.transform(_.objz('a)).using(_      .ints('h) ).check(GrabOut)
if(false) GrabIn.transform(_.objz('a)).using(_.grab('h)).check(GrabOut) // FIXME?
  }
